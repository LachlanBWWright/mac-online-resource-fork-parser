...
describe("ResourceForkParser", () => {
  ...
  it("renders the main interface components", () => {
    ...
    expect(
      screen.getByRole("button", { name: /Convert from JSON/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Load EarthFarm Sample/i }),
    ).toBeInTheDocument();
  });
  ...
  it("has save/load specifications in a collapsible section", () => {
    ...
    const cardContainer =
      saveLoadSection.closest("[data-state]") ||
      saveLoadSection.closest(".cursor-pointer");
  ...
  it("loads EarthFarm sample and displays four-letter codes", async () => {
    ...
    const loadButton = screen.getByRole("button", {
      name: /Load EarthFarm Sample/i,
    });
    fireEvent.click(loadButton);

    await waitFor(() => {
      expect(
        screen.getByText("Four-Letter Code Specifications"),
      ).toBeInTheDocument();
    });
